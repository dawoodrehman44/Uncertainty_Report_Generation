...
def main():
    ...
    CONFIG = {
        ...
        'chexpert_csv': "/mnt/External/Seagate/dawood/datasets/mimic-cxr/mimic-cxr-2.0.0-chexpert.csv",
        'split_csv': "/mnt/External/Seagate/dawood/datasets/mimic-cxr/mimic-cxr-2.0.0-split.csv",
        'metadata_csv': "/mnt/External/Seagate/dawood/datasets/mimic-cxr/mimic-cxr-2.0.0-metadata.csv",
        'report_dir': "/mnt/External/Seagate/dawood/datasets/mimic-cxr/report",
        
        # Output files
        'output_dir': "/mnt/External/Seagate/dawood/datasets/mimic-cxr/cleaned",
        'train_output': "mimic_clean_train.csv",
        'valid_output': "mimic_clean_valid.csv",
        'report_output': "cleaning_report.txt",
        
        # ✅ CHANGED: Use more data
        'max_samples': 200000,  # Process up to 200K samples (set to None for ALL)
    ...
    # ========================================================================
    print("📂 Loading raw MIMIC-CXR data...")
    
    labels_df = pd.read_csv(CONFIG['chexpert_csv'])
    splits_df = pd.read_csv(CONFIG['split_csv'])
    metadata_df = pd.read_csv(CONFIG['metadata_csv'])
    
    # Convert IDs to strings
    for df in [labels_df, splits_df, metadata_df]:
        for col in ['subject_id', 'study_id', 'dicom_id']:
            if col in df.columns:
                df[col] = df[col].astype(str)
    
    # Merge
    data_df = labels_df.merge(
        splits_df[['subject_id', 'study_id', 'split']],
    ...
    print(f"  Loaded {len(data_df)} studies")
    ...
    if os.path.exists(CONFIG['report_dir']):
        txt_files = [f for f in os.listdir(CONFIG['report_dir']) if f.endswith('.txt')]
        print(f"  Found {len(txt_files)} report files")
        
        for fname in tqdm(txt_files, desc="Reading reports", ncols=80):
            if fname.startswith('s') and fname.endswith('.txt'):
                ...
                try:
                    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                        text = f.read().strip()
                    if text:
                        report_data.append({'study_id': study_id, 'report_text': text})
                except:
                    continue
    ...
    # Merge reports
    data_df = data_df.merge(reports_df[['study_id', 'report_text']], on='study_id', how='left')
    data_df['Report'] = data_df.get('report_text', '').fillna('')
    
    print(f"  Merged {len(reports_df)} reports")
    ...
    # Only keep train and validate (exclude test)
    data_df_filtered = data_df_matched[
        data_df_matched['split'].isin(['train', 'validate'])
    ].reset_index(drop=True)
    
    print(f"  Kept {len(data_df_filtered)} samples (train + validate)")
    ...
    all_clean = cleaner.process_dataframe(data_df_filtered, split_name='all')
    ...
    all_scored = cleaner.filter_by_clinical_relevance(all_dedup, min_score=0.35)  # ✅ Relaxed from 0.4
    ...
    keep_cols = [
        ...
        'Pneumonia', 'Atelectasis', 'Pneumothorax', 'Pleural Effusion',
        'Pleural Other', 'Fracture', 'Support Devices',
        'Findings_Clean', 'clinical_relevance_score'
    ]
    ...
    train_save = train_final[train_keep_cols].copy()
    valid_save = valid_final[valid_keep_cols].copy()
